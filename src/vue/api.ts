...
export async function expectJson<T>(response: Response, notifyUnauthorized = false): Promise<T> {
  // SAFETY: Callers supply the response type generated by Hono's typed RPC client.
  ...
  if (!response.ok) {
    ...
    // SAFETY: The API error contract has an optional string message property.
    throw new ApiError(
  ...
  // SAFETY: Successful Hono responses match the type supplied by the typed endpoint caller.
  return body as T;
}
