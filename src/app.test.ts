...
describe("application", () => {
  it("serves health, byte ranges, and production routes", async () => {
    const directory = await createTemporaryDirectory("playlist-app-");
    ...
    const configuration = createConfiguration({
      ...
      AUTH_SETUP_TOKEN: "playlist-app-test-setup-token",
    });
    ...
    const rejectedSetup = await app.request("/api/auth/password", {
      ...
      body: JSON.stringify({
        password: "playlist-test-password",
        confirmPassword: "playlist-test-password",
    ...
    const setup = await app.request("/api/auth/password", {
      ...
      body: JSON.stringify({
        password: "playlist-test-password",
        confirmPassword: "playlist-test-password",
        setupToken: "playlist-app-test-setup-token",
      }),
    });
    ...
    const login = await app.request("/api/auth", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ password: "playlist-test-password" }),
    });
    ...
    const lessonDetail = await app.request(`/api/catalog/videos/${"b".repeat(24)}`, {
      headers: { cookie: cookie! },
    });
    ...
    for (const catalogPath of [
      "/api/catalog/playlists/not-an-id",
      "/api/catalog/videos/not-an-id",
      `/api/catalog/playlists/${"c".repeat(24)}`,
      `/api/catalog/videos/${"d".repeat(24)}`,
    ]) {
